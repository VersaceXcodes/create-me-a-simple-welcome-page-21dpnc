...
const isTunnelEnvironment =
  Boolean(process.env.TUNNEL_DOMAIN) ||
  authOriginCandidates.some((origin) => isTrustedTunnelOrigin(origin) || isTrustedModalOrigin(origin));
...
const auth = betterAuth({
  ...
  advanced: {
    ...
    defaultCookieAttributes: (isProduction || isTunnelEnvironment)
      ? {
          sameSite: 'none',
          secure: true,
          partitioned: true,
